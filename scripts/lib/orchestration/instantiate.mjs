...
// templates (DD-5, C3).
//
// Same marker vocabulary as setup (<!-- fcore:slot:name -->, kebab-case)
...
//   - any fcore:slot comment that is not a well-formed kebab-case marker
...
export const INLINE_SLOT_RE = /<!--\s*fcore:slot:([a-z0-9-]+)\s*-->/g;

// Any HTML comment mentioning fcore:slot — superset of INLINE_SLOT_RE used
// to catch malformed markers (bad casing, stray characters) that strict
// matching would silently leave in the output.
const ANY_SLOT_COMMENT_RE = /<!--[^>]*fcore:slot[^>]*-->/g;
const WELL_FORMED_RE = /^<!--\s*fcore:slot:[a-z0-9-]+\s*-->$/;
