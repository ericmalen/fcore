...
test('install-setup ships base-check verbatim from .claude/skills', () => {
  ...
  try {
    const r = spawnSync(process.execPath,
      [join(KIT, 'scripts/install-setup.mjs'), target], { encoding: 'utf8' });
...
test('install-setup ships only the five setup scripts + lib, no kit-dev tooling', () => {
  const target = makeGitRepo();
  try {
    const r = spawnSync(process.execPath,
      [join(KIT, 'scripts/install-setup.mjs'), target], { encoding: 'utf8' });
...
test('install-setup ships orchestration lifecycle skills, keeps discovery/generation kit-only', () => {
  const target = makeGitRepo();
  try {
    const r = spawnSync(process.execPath,
      [join(KIT, 'scripts/install-setup.mjs'), target], { encoding: 'utf8' });
