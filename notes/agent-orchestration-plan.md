# FleetCore orchestration — Complete Build Plan

Repo-agnostic agent/skill orchestration system, built in `fcore` and distributed
through the existing setup installer (`scripts/install-setup.mjs` allowlist).
Discovery runs from an open FleetCore clone against a target path — same pattern as
`fcore-onboard`. Verification targets are the repo fixtures (§9.5):
...
progress — lifecycle skills (`checklist-intake`, `log-report`, `eval-runner`) ship to every
adopted repo; `spec/target-layout.md` documents the conditional orchestration
layer; discovery/generation meta-assets deliberately stay FleetCore-side (the
`fcore-onboard` pattern). F1-remaining (self-serve docs), F3 (tracker bridge,
...
| DD-9  | Tests: Node built-in test runner via `npm test` (`node --test "test/**/*.test.mjs"`, Node ≥ 22; shipped scripts stay Node ≥ 20), pure functions only (validators, slot-filling, renderers, parsers). Agent behavior is checked by evals, not unit tests.                                        | Matches FleetCore's existing test setup and testing scope.                                                               |
| DD-10 | Every shipped bug or substantive review finding becomes a checklist item via the `checklist-intake` skill (§7, Phase E). No exceptions.                                                                            | The compounding quality loop; FleetCore's long-term asset.                                                               |
...
| DD-13 | The per-target **generation manifest** (`docs/orchestration/generation-manifest.json`: generated file → template id, pinned template version, SHA) is orchestration-scoped state written by the scaffolder. It is **not** the setup routing manifest and **not** a revival of v1 sync machinery — its scope is exclusively FleetCore-generated orchestration files. **The manifest — not the blueprint — owns the version pins:** the scaffolder owns the manifest entirely, Discovery owns `blueprint.json` entirely; one writer per artifact, no machine-managed fields inside Discovery outputs. | Drift detection (E3) and update (C5) need a recorded baseline; keeping it separate preserves the v2 "no sync layer" stance for setup; single ownership keeps re-scaffolds from mutating their own input. |
...
- Meta-skills (13, `.claude/skills/`): `structure-detector`, `dependency-mapper`, `convention-detector`, `interview-guide`, `blueprint-generator`, `handoff-validator`, `agent-instantiator`, `skill-instantiator`, `eval-runner`, `drift-checker`, `checklist-intake`, `log-report`, `tracker-sync` (F3)
...
rides into targets inside `.claude/fcore-onboard/templates/` via the existing
wholesale copy in `install-setup.mjs`. Pre-F1 that ride-along is harmless
dead weight: `.claude/fcore-onboard/` is setup-time tooling that
`fcore-verify` removes before merge, so non-piloting adoptees never keep it.
...
│   ├── agents/                    # *.template.md with <!-- fcore:slot:name --> markers
│   ├── skills/
...
developing FleetCore — that is wanted here, since discovery/generation runs from a
FleetCore clone against targets and fixtures (the `fcore-onboard` pattern). Only
...
**Goal:** point FleetCore at any repo, get a validated blueprint out. Semantic work lives here.
...
| Step | Work                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Deliverable                           | Acceptance                                                                                                                                               |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C1   | Author templates: orchestrator + 6 specialists + generic fallback, each with `<!-- fcore:slot:name -->` markers for: repo paths, stack names, test commands, conventions, doc references, model tier, turn limit                                                                                                                                                                                                                                                                                                                                                                                                                                                | `templates/orchestration/agents/*.template.md` | Slot lint: every slot name appears in blueprint schema; no orphan slots                                                                                  |
...
| Step | Work                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Deliverable                              | Acceptance                                                                                                                                               |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| E1   | `checklist-intake` skill: input = bug report or review finding; output = appended item in `docs/orchestration/checklists/review-checklist.md` (format: `- [ ] CHK-### (src: BUG-###/PR-###): <check>`); code-reviewer template references this checklist via lazy-load                                                                                                                                                                                                                                                                  | Skill + checklist file + template update | Feed it 2 seeded maxi-repo findings → 2 well-formed checklist items the reviewer applies on next run                                                           |
...
| Step | Work                                                                                                                                                                                                                                                                                                                                                       | Deliverable         | Acceptance                                                  |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ----------------------------------------------------------- |
| F1   | **Partial (2026-06-11).** Extend `scripts/install-setup.mjs` with orchestration lifecycle skills (`checklist-intake`, `log-report`, `eval-runner`); add the conditional **orchestration layer** section to `spec/target-layout.md`. `templates/orchestration/` and `scripts/lib/orchestration/` ride along with the wholesale `templates/` + `scripts/lib/` copies (setup-time only; removed by `fcore-verify`). **Deferred:** shipping discovery/generation meta-agents/skills to targets — they stay FleetCore-side and run from an FleetCore clone (same pattern as `fcore-onboard`; see `install-setup.mjs` header comment) | Allowlist entries + tests + spec section | **Done:** lifecycle skills install verbatim; `spec/target-layout.md` covers generated surfaces. **Remaining:** self-serve docs — per-session expected outputs, time budgets, Gate 1/2 checklists, pre-flight checklist in `docs/how-to/orchestration-guide.md`; failure modes + Copilot inline fallback in `docs/how-to/orchestration-troubleshooting.md` |
| F2   | Teammate pilot: one colleague runs Discovery → Generation on a second **real** repo using only the docs. Capture friction → checklist-intake items. Protocol, intervention-counting rules, and report template live in `notes/orchestration-pilot.md`; completed reports land in `reports/orchestration-pilot-<repo>-<date>.md`                                                                                                                                                                                                                    | Protocol doc + pilot report        | Blueprint + generated agents with ≤2 interventions from you; every friction row routed per the triage taxonomy |
...
**Phase F done when:** ≥2 teammates self-serve FleetCore and one scheduled pipeline run has shipped a PR.
...
- Templates: `<name>.template.md`; slots `<!-- fcore:slot:name -->`, kebab-case (DD-5)
...
- Meta-skill trigger descriptions must be **orchestration-scoped** — generic names (`checklist-intake`, `log-report`, `drift-checker`) collide with project skills post-F1; the description, not the name, is what prevents mis-triggering
...
- Dependencies declared in manifests only — never installed; stub `build`
  scripts and Node-builtin tests keep FleetCore zero-dependency; `ui` and `api`
