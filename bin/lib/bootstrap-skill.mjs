...
// TARGET's .claude/skills/ so the user types /fcore-bootstrap instead
// of carrying a pasted prompt across tools. The skill is untracked, orders
// its own deletion first (restoring the clean tree the fcore-* preconditions
...
export const BOOTSTRAP_SKILL_DIR = join('.claude', 'skills', 'fcore-bootstrap');
...
function skillContent({ command, checkoutPath, targetPath, dev }) {
  return `---
name: fcore-bootstrap
description: One-shot launcher dropped by \`npx github:<owner>/fcore#<tag> ${command}\`. Use when the user types /fcore-bootstrap or asks to start the staged fcore ${command} flow. Deletes itself first.
---

# fcore-bootstrap (one-shot launcher)

Dropped by \`npx github:<owner>/fcore#<tag> ${command}\`. Execute top to bottom:
...
2. Verify the fcore checkout still exists at \`${checkoutPath}\`. If it is
   missing (e.g. removed by \`fcore cache prune\`), STOP and tell the
