...
or standard, rejecting an obvious approach. Not ADR material: reversible
...
implementing embodies such a decision and no ADR exists, offer to write
one — don't wait to be asked.

## Format (context → decision → consequences)

File: `docs/adr/NNNN-short-slug.md`, NNNN zero-padded, append-only.
Skeleton in [templates](templates.md). Sections:

- **Status**: Accepted | Superseded by ADR-NNNN. Never edit a decision
  after acceptance — write a new ADR that supersedes it and update only
...
- **Consequences**: what becomes easier, what becomes harder, what is now
  constrained — including the negative ones. An ADR with only upsides is
...
code, the human), date the ADR with today's date noting the decision's
