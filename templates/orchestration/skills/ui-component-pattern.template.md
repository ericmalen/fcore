...
description: Procedure for adding or modifying a component in the <!-- agent-base:slot:stack --> layer at <!-- agent-base:slot:layer-path -->. Use when a UI component in that layer must be created or changed and should match the codebase's existing component pattern. Not for API endpoints, styling-system overhauls, or layers outside <!-- agent-base:slot:layer-path -->.
---
...
1. Find an existing component as the pattern. Pick one under
   `<!-- agent-base:slot:layer-path -->` that is closest in kind to what you are
