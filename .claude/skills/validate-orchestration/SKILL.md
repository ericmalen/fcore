...
1. Workspace: `WORK=~/fcore-validation/orch-<YYYYMMDD-HHMM>`; record
   FleetCore SHA. Never run inside the fcore checkout.
2. Build the fixture once:
   `node <fcore>/scripts/build-orchestrated-fixture.mjs $WORK/base --seed-ref --seed-blocked --install`
...
4. **feature-orchestrator goldens** — for each golden in
   ...
   `node <fcore>/scripts/validate-orchestration-assert.mjs --dir $WORK/orch-<id> --task <T-id> --expect <pruned|done-with-ref|blocked> --base <fixture base sha> --json`
...
5. **Sabotage** (mechanical, no LLM sessions) — in a fresh `cp -R $WORK/base
   ...
   - Strip the routing region from `AGENTS.md` (delete between the
     `fcore:orchestration-routing` markers) → `node
     <fcore>/scripts/audit.mjs --root $WORK/sabotage` must report R-56.
   - Hand-edit a generated agent file (e.g. append a line to
     `.claude/agents/api-engineer.md`) → the FleetCore `drift-checker` skill
     (or `findUserEdits` from `scripts/lib/orchestration/scaffold.mjs`) must
     report it as USER-EDIT.
   Catch-rate = n/2.
6. Report → `<fcore>/reports/orchestration-validation-<date>.md`
   ...
   sabotage catch-rate, environment (model, FleetCore SHA), any escalations.
...
- Never run fixtures inside the fcore checkout; always a separate work dir.
