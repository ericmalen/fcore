...
## docs-manager setup (bootstrap — run at setup)
...
2. Add the docs section to AGENTS.md (tool-neutral — any agent must be able
   ...
   ```
   ## Documentation
   Conventions: .claude/skills/docs-manager/SKILL.md (standard, types, rules).
...
4. Tier T2+ only — wire enforcement (skip both at T1 by design):
   - Merge into `.claude/settings.json` hooks (read by both tools):
     SessionStart → `node .claude/skills/docs-manager/scripts/docs-nudge.mjs session-start`
     Stop → `node .claude/skills/docs-manager/scripts/docs-nudge.mjs stop`
   - If the repo has CI, offer the docs-impact gate: copy the matching
     template (FleetCore `templates/ci/docs-impact.github.yml` →
...
## docs-manager restructure (sort existing docs into quadrants)
...
6. Verify links resolve — run `node .claude/skills/docs-manager/scripts/check-links.mjs`
...
## docs-manager catch-up (human changed code without AI)
