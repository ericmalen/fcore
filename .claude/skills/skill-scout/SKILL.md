---
name: skill-scout
description: Pre-authoring dedup — search existing skill sources (this repo's .claude/skills, user-level ~/.claude, marketplaces, GitHub, web) before creating a new skill, so existing work is reused instead of duplicated. Use when about to create, build, fork, or find a skill for a workflow, or when asked "is there a skill for X". Pairs with skill-creator. NOT for growing the vendored stack-skill catalog in templates/stack-skills/ — that is stack-skill-scout.
metadata:
  origin: community
---
...
```bash
find .claude/skills ~/.claude/skills -maxdepth 2 -name SKILL.md 2>/dev/null | grep -iE "keyword|synonym"
...
| --- | --- | --- | --- | --- |
| 1 | eval-runner | Local .claude/skills | Runs golden evals against generated agents | No drift detection, evals only |
| 2 | drift-checker | Local .claude/skills | Re-instantiates manifest entries, diffs digests | Doesn't check runtime behavior |
| 3 | agent-regression-kit | GitHub | Generic agent regression harness, recent commits | Needs security review |
```
...
`drift-checker`; it detects when generated agents diverge from their templates,
but it does not re-run their golden evals — `eval-runner` covers that half. I
can use the two together as-is, fork one into a combined variant, or create a
fresh skill.
```
...
- `skill-creator` - Authoring loop once "create fresh" is the chosen path.
- `stack-skill-scout` - Scouts framework-practice skills for the vendored
  stack-skill catalog (`templates/stack-skills/`); a different pipeline — do
  not use this skill for catalog growth.
