...
# fcore-onboard (setup entry point + orchestrator)

You are executing this file in one of two modes — determine which first:

- **Skill mode** — this fcore checkout is open in your tool and the user
  invoked `/fcore-onboard <target-path>`. Orchestrate all four phases; the user
...
Either way: the target must be a git repository and must NOT be this fcore
checkout. Run all commands yourself via your shell tool — never ask the user to
run commands. This skill is FleetCore-side only: the installer never ships it into
targets. The setup happens on a branch (`fcore-onboard`); the user's
...
3. Install the setup tooling into the project and commit it:
   ...
   ```sh
   node <path-to-this-fcore-checkout>/scripts/install-setup.mjs <project-path>
   cd <project-path>
   git add -A
   git commit --no-verify -m "chore: fcore setup tooling"
   ```
...
5. Run the four phases. **Claude Code (subagent orchestration):** dispatch
   ...
   fcore-verify must also be fresh subagents — never reuse a phase context.
   **Copilot:** ATTEMPT the same subagent orchestration first (FleetCore's
   ...
   fcore-inventory inline, then hand the user the per-phase instructions
...
Read `.claude/skills/fcore-inventory/SKILL.md` in the project and execute
...
dispatch lets the four phases run as one `fcore-onboard` command, with the manual
"start a fresh session and run `fcore-plan`" path as the fallback.
