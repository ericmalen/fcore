...
silent when the repo is clean or no Agent Base checkout is reachable.
...
finds Agent Base via `$AI_KIT_HOME`, `.claude/agent-base-setup/` (during setup),
or `~/tools/agent-base`:
...
in every case, so a missing Agent Base checkout or a parse error is silently ignored.
